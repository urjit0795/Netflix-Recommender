...
content_similarity = build_content_similarity(
    movies
)
...
@st.cache_resource
def get_semantic_search_resources(
    movies_df,
):
    """
    Load the Sentence Transformer model and cached FAISS index
    over movie embeddings.
    """
...
@st.cache_resource
def build_collaborative_similarity(
    ratings_df,
):
...
user_movie_matrix, movie_similarity_df = (
...
def hybrid_recommend(
    ...
    if (
        matches.empty
        or user_id
        not in user_movie_matrix.index
    ):
        return pd.DataFrame()
...
st.sidebar.write(
    ...
    "using movie ratings and metadata."
)
...
if option == "Popular Movies":
    ...
    st.info(
        "This method recommends movies with high average ratings, "
        "while filtering out movies with very few ratings."
    )
...
# ---------------------------------------------------------
    ...
    st.info(
        "Content-based filtering recommends movies similar "
        "to a selected movie using title, genre, and release year."
    )
...
# ---------------------------------------------------------
    ...
    st.info(
        ...
        "rating patterns from similar users and movies."
    )
...
# ---------------------------------------------------------
    ...
    st.info(
        "Hybrid recommendation combines content similarity "
        "and collaborative filtering. The resulting candidates "
        "can then be reranked using an LLM based on the user's "
        "preference history."
    )
    ...
    with st.expander(
        "🧠 User Context for LLM Reranking"
    ):
        st.text(
            user_context
        )
    ...
    st.write(
        f"Collaborative filtering weight: "
        f"**{cf_weight:.2f}**"
    )
    ...
    with st.expander(
        "🎬 Candidate Context for LLM Reranking"
    ):
        st.text(
            candidate_context
        )
...
# ---------------------------------------------------------
    ...
    st.info(
        ...
        "(`all-MiniLM-L6-v2`) to embed movie metadata and your "
        "query into the same vector space, then retrieves the "
        "closest matches using a FAISS vector index."
    )

    query = st.text_input(
        "Describe the kind of movie you're looking for",
        placeholder=(
            "e.g. gritty crime thriller from the 90s"
        ),
    )
...
# ---------------------------------------------------------
    ...
    st.info(
        "This is a Retrieval-Augmented Generation pipeline: "
        "your message retrieves candidate movies from the "
        "FAISS vector index, then an LLM recommends from only "
        "those retrieved candidates and explains why."
    )
    ...
    user_message = st.chat_input(
        "e.g. Show me a feel-good movie for a rainy day"
    )
